document.addEventListener('DOMContentLoaded', () => {
            ...
            if (document.pictureInPictureElement) {
                ...
                if (videoElement.requestPictureInPicture) {
                    videoElement.requestPictureInPicture()
                        .catch(error => console.error('PiP request failed:', error));
                }
            }
            resetActivityTimeout(); 
        });
    ...
    if ('mediaSession' in navigator) {
        navigator.mediaSession.metadata = new MediaMetadata({
            title: 'Cyrus AI Sample Video',
            artist: 'Cyrus AI',
            album: 'Cyrus Recommendations',
            artwork: [
        ...
        navigator.mediaSession.setActionHandler('play', () => { videoElement.play(); resetActivityTimeout(); });
        navigator.mediaSession.setActionHandler('pause', () => { videoElement.pause(); resetActivityTimeout(); });
        ...
        navigator.mediaSession.setActionHandler('seekforward', (event) => {
            const skipTime = event.seekOffset || 10;
            videoElement.currentTime = Math.min(videoElement.duration, videoElement.currentTime + skipTime);
            resetActivityTimeout();
        });
    ...
    let _startX, _startY; // Store initial mouse/touch clientX/Y for click detection
    ...
    const CLICK_THRESHOLD = 5; // Max pixels moved to still count as a click (reduced from 10)
    ...
    function startDrag(e, element) {
        // Prevent default browser behavior on mousedown/touchstart if it's a drag target
        // This is crucial for draggable elements
        ...
        // Store initial touch/mouse position for click detection later
        _startX = clientX;
        _startY = clientY;
        ...
        console.log('START DRAG:', element.id, 'Initial Mouse:', _startX, _startY); // Debugging
    }

    function doDrag(e) {
        if (!_isDragging || !_draggedElement) return;

        // IMPORTANT: Prevent default browser actions (like scrolling or text selection)
        // This MUST be here and be consistent.
        e.preventDefault(); 
        ...
        // console.log('DO DRAG:', _draggedElement.id, 'New Pos:', newX, newY); // Debugging (can be chatty)
    }
    ...
    function stopDrag(e) {
        if (_isDragging && _draggedElement) {
            ...
            const clientX = e.changedTouches ? e.changedTouches[0].clientX : e.clientX; // Get final position for click check
            ...
            console.log('STOP DRAG:', _draggedElement.id, 'Distance moved:', distanceMoved, 'Click Threshold:', CLICK_THRESHOLD); // Debugging

            // Only trigger click/toggle if it wasn't a drag motion
            if (_draggedElement === aiCircle && distanceMoved < CLICK_THRESHOLD) {
                console.log('Recognized as a TAP on AI Circle. Toggling interface.'); // Debugging
    ...
    document.addEventListener('touchmove', doDrag, { passive: false }); // passive: false is key for e.preventDefault() on touch
    ...
    function toggleCyrusInterface() {
        console.log('TOGGLE INTERFACE CALLED. Current hidden status:', cyrusInterface.classList.contains('hidden')); // Debugging
        if (cyrusInterface.classList.contains('hidden')) { 
            cyrusInterface.classList.remove('hidden'); // SHOW interface
            userInput.focus();
            ...
            cyrusInterface.classList.add('hidden'); // HIDE interface
    ...
    cyrusLog.addEventListener('click', resetActivityTimeout);
    
    // Initial assignment of 'hidden' class to cyrusInterface
    cyrusInterface.classList.add('hidden');
    ...
    // Ensure interface starts hidden before loading saved position
    cyrusInterface.classList.add('hidden'); 
    
    loadElementPosition(aiCircle);
    loadElementPosition(cyrusInterface);

    // Initial positioning for the Cyrus interface if it hasn't been moved AND it's hidden
    // This helps place it neatly next to the AI circle on first load if no position is saved.
    if (localStorage.getItem('cyrusInterfaceX') === null) {
        const aiRect = aiCircle.getBoundingClientRect();
        const initialInterfaceLeft = aiRect.right + 20;
        ...
        // If interface position is loaded, ensure the offset is calculated based on current positions
        const aiRect = aiCircle.getBoundingClientRect();
